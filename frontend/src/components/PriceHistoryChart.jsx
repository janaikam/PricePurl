...
const PriceHistoryChart = ({ priceHistory, lowestPriceAt }) => {
  if (!priceHistory?.length) {
    return (
      <div style={{ padding: '24px', border: '1px dashed #b8a6a0', borderRadius: '16px' }}>
  ...
  return (
    <div style={{ border: '1px solid #d7c2ba', borderRadius: '18px', padding: '16px', backgroundColor: '#fffaf8' }}>
      <svg viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} role="img" aria-label="Price history chart" style={{ width: '100%', height: 'auto', display: 'block' }}>
        <line x1={CHART_PADDING} y1={CHART_PADDING} x2={CHART_PADDING} y2={CHART_HEIGHT - CHART_PADDING} stroke="#d7c2ba" strokeWidth="1" />
        <line x1={CHART_PADDING} y1={CHART_HEIGHT - CHART_PADDING} x2={CHART_WIDTH - CHART_PADDING} y2={CHART_HEIGHT - CHART_PADDING} stroke="#d7c2ba" strokeWidth="1" />
        <path d={buildPath(points)} fill="none" stroke="#1f6f5f" strokeWidth="4" strokeLinejoin="round" strokeLinecap="round" />
        {points.map((point) => (
          <circle
            key={point.id}
            cx={point.x}
            ...
            fill={point.isLowest ? '#c94f3d' : '#f6b267'}
            stroke="#ffffff"
            strokeWidth="2"
          />
        ))}
        <text x={CHART_PADDING} y={CHART_PADDING - 6} fill="#6a4f48" fontSize="14">
          ${maxValue.toFixed(2)}
        </text>
        <text x={CHART_PADDING} y={CHART_HEIGHT - 6} fill="#6a4f48" fontSize="14">
          ${minValue.toFixed(2)}
        </text>
        <text x={lowestPoint.x} y={Math.max(lowestPoint.y - 14, 14)} fill="#8f2d1e" fontSize="14" textAnchor="middle">
          Lowest
        </text>
      </svg>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', marginTop: '12px', fontSize: '0.95rem', color: '#6a4f48' }}>
